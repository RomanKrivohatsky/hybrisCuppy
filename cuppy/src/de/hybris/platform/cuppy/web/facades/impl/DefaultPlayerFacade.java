...
import de.hybris.platform.cuppy.services.*;
...
public class DefaultPlayerFacade implements PlayerFacade
{
	...
	@Override
	public List<ChatMessageData> findMessages()
	{
		List<ChatMessageModel> messages = chatService.findMessages();

		return populateMessageData(messages);
	}

	@Override
	public List<ChatMessageData> findMessagesFromDate(Date from)
	{
		List<ChatMessageModel> messages = chatService.findMessagesFromDate(from);

		return populateMessageData(messages);
	}

	@Override
	public List<ChatMessageData> findMessagesByDate(Date from, Date to)
	{
		List<ChatMessageModel> messages = chatService.findMessagesByDate(from, to);

		return populateMessageData(messages);
	}


	List<ChatMessageData> populateMessageData (List<ChatMessageModel>  messages) {

		if (messages == null || messages.size()<1) {
			return Collections.emptyList();
		}

		Iterator<ChatMessageModel> it = messages.listIterator();
		List<ChatMessageData> messageDatas = new ArrayList<ChatMessageData>();
		while (it.hasNext()) {
			ChatMessageModel mes = it.next();
			ChatMessageData messageData = new ChatMessageData();
			messageData.setContent(mes.getContent());
			PlayerModel player = mes.getPlayer();
			if (player!=null) {
				messageData.setPlayerName(player.getName());
				messageData.setUiID(player.getUid());
			}
			messageData.setCreationDate(mes.getCreationtime());
